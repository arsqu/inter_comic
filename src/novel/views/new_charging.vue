<template>
  <div class="new_charging">
    <div class="balance" v-if="isLogin">
      ...
      <span>{{ balance }} {{ $t("common.priceUnit") }}</span>
    </div>
    <!-- recharge -->
    <div class="recharge_box" @keyup.13="askRecharge">
      <div v-show="showInput" :class="{ inpBox: isLogin }">
        <input
          type="text"
          v-model="money"
          @input="clearVal"
      ...
      <div class="paybox" v-show="payment">
        <div class="payfor">
          ...
          <span
            :class="[isLoad ? 'opa' : '', 'pay_txt btn_primary pay']"
            @click="askToPay"
          >
...
.new_charging
  ...
  .balance
    ...
    align-items center
    background url('~x/image/bg_charging.png')repeat-x

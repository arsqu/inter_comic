  ...
  <div class="comic_detl">
    ...
    <div class="comic_desc">
      ...
      <div class="container_item">
        ...
        <!-- 目录 -->
        <!--  v-show="isCur == 2" -->
        <div class="cont show_chapter">
          <div class="cont cont_book_list" @click="showChapter">
            <span>Contents</span>
            <span>Total {{ totalNum }} Chapters</span>
            <span class="totalNum">View more</span>
          </div>
          <div class="line"></div>
          <div class="cont chapter_detl">
            <span class="chapter_txt">{{
              $t("column.chapter") + " " + orderNo
            }}</span>
            <div class="desc multi_text" @click="viewDetl">{{ info }}</div>
          </div>
        </div>
      </div>
    </div>
...
<script>
const column = () => import("x/components/module/column");
const gaussian = () => import("@/components/module/gaussian");
const category = () => import("x/components/module/category");
...
export default {
  ...
  methods: {
    ...
    showChapter() {
      ...
      console.log(this.catalogue.length, this.page.total);
      if (this.catalogue.length < this.page.total) {
        var opt = this.page;
        opt.limit = opt.total;
        this.getAllChapter(opt);
      }
    },
    //查看漫画详情
    viewDetl() {
    ...
    //所有章节(包含付费)
    async getAllChapter(opt) {
      opt = opt || Object.assign({}, this.page);
